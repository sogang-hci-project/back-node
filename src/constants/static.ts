// export default {
//   introduce: `Greetings, my dear friend! Welcome to my painting Guernica.
//   I, Pablo Picasso, extend my warmest salutations to you. With my brush as my voice and colors as my language, I endeavor to create a world where imagination knows no bounds.
//   Now, can you introduce me about yourself? Maybe about your name or interest.`,
//   what: `I see, Lets move on to the questions. Questions can harness the power of visual imagery to enhance understanding of the art.
//   I'll ask you a couple of questions to help you find captivating features of my painting. Will you join me?`,
//   first: "In the painting, what do you think is going on?",
//   second: "What else can you find in the painting?",
//   third: "Is there anything other that catches your attention?",
//   evaluate:
//     "Well done, my dear friend. Our journey within the painting Guernica has now ended. Tell me with honest hearts. Did you enjoy our conversation?",
//   introduceKorean: `안녕, 친구! 제 그림 게르니카에 오신 것을 환영합니다.
//   저 파블로 피카소가 여러분에게 따뜻한 인사를 전합니다. 저는 붓을 제 목소리로, 색을 제 언어로 삼아 상상력의 한계가 없는 세상을 만들기 위해 노력합니다.
//   이제 본인에 대해 소개해 주시겠어요? 이름이나 관심사에 대해 소개해 주세요.`,
//   whatKorean: `그렇군요, 이제 질문으로 넘어가 보겠습니다. 질문은 시각적 사고의 힘을 활용하여 예술에 대한 이해를 높일 수 있습니다.
//   제 그림의 매력적인 특징을 찾는 데 도움이 되는 몇 가지 질문을 해보겠습니다. 함께 하시겠어요?`,
//   firstKorean: `그림에서 무슨 일이 일어나고 있다고 생각하시나요?`,
//   secondKorean: `그림에서 또 어떤 것을 찾을 수 있나요?`,
//   thirdKorean: `그 외에 관심을 끄는 다른 것이 있나요?`,
//   evaluateKorean: `잘했어, 친구. 이제 게르니카 그림 속 우리의 여정이 끝났습니다. 솔직한 마음으로 말해 주세요. 대화가 즐거웠나요?`,
// };

export default {
  GREETING_ONE_EN: `
  Hello! I am Pablo Picasso, a young man who paints. Who are you?
  `,
  GREETING_TWO_EN: `I see, why don't you join me in a conversation and look at the pictures together?`,

  GREETING_ONE_KO: `안녕하세요! 저는 그림을 그리는 청년 파블로 피카소입니다. 당신은 누구인가요?`,
  GREETING_TWO_KO: `그렇군요, 저와 대화를 하며 그림을 함께 감상해보시겠습니까?`,

  VTS_ONE_EN: `What do you think is happening in the picture?`,
  VTS_TWO_EN: `What else can you find in the picture?`,
  VTS_THREE_EN: `Is there anything else that catches your attention?`,
  VTS_ONE_KO: `그림에서 무슨 일이 일어나고 있다고 생각하시나요?`,
  VTS_TWO_KO: `그림에서 또 어떤 것을 찾을 수 있나요?`,
  VTS_THREE_KO: `그 외에 관심을 끄는 다른 것이 있나요?`,
};
